...
export const getVisualNovelReleases = async (interaction) => {
    ...
    visualNovelReleases.map((release) => {
        switch (release.languages[0].lang) {
            case 'en':
                release_EN += `[${release.released !== 'TBA' ? convertDate(release.released) : 'TBA'} | ${release.platforms[0]} | ${release.title} | ${release.patch === true ? '(Patch)' : ''}](${release.extlinks[0] ? release.extlinks[0].url : ''})\n`;
                break;
            case 'ja':
                release_JP += `[${release.released !== 'TBA' ? convertDate(release.released) : 'TBA'} | ${release.platforms[0]} | ${release.title} | ${release.patch === true ? '(Patch)' : ''}](${release.extlinks[0] ? release.extlinks[0].url : ''})\n`;
                break;
            case 'vi':
                release_VI += `[${release.released !== 'TBA' ? convertDate(release.released) : 'TBA'} | ${release.platforms[0]} | ${release.title} | ${release.patch === true ? '(Patch)' : ''}](${release.extlinks[0] ? release.extlinks[0].url : ''})\n`;
                break;
            default:
                break;
        }
    ...
    const embed = new EmbedBuilder()
        ...
        .setTimestamp().setDescription(` ${release_EN !== '' ? '**English :flag_gb: **\n' + release_EN : ''}\n 
